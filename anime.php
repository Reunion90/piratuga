<?php
    ...
    $result = mysqli_query($connection, "SELECT * FROM animes WHERE id = '$id'") or header("Location: filmes.php");
    $hasrow = 0;
        ...
        <section id="banner" class="clearfix"><div class="bg-img"> <?php if($banner != ""){echo '<img src="'.$banner.'"/>';}?></div>
            <div id="banner_content_wrapper">
                ...
                <div id="content">
                    ...
                    <p class="info"><em>Título Alternativo: </em><?php echo $alternative ?></p>
                </div>
            </div>
        </section>
        <section id="episodes">
            <?php 
                require_once("import/connection.php");
                $seasons = array();
                $sql = mysqli_query($connection, "SELECT * FROM anime_eps WHERE (anime_id = '$id')");
                if($sql != FALSE){
                ...
                foreach($seasons as $season){
                    ...
                    <?php 
                    require_once("import/connection.php");
                    $query = mysqli_query($connection, "SELECT * FROM anime_eps WHERE (anime_id = '$id' AND season = '$season')");
                    while ($row = mysqli_fetch_assoc($query)){
                        ?>
                        <a href="epanime.php?f=<?php echo $row['id']?>"><div class="post">
